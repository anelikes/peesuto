...
Core 与独立的 Pocket Motion；最终交付完全移除 Tauri 和界面 WebView，不使用 Electron。**
...
已实现当前剪贴板直跑快捷键、非抢焦点状态浮窗和有限任务生命周期事件。`app/` 仍保留用于对照，
尚未完成全部替换。M0–M8 保留历史记录；N1 已完成，N2–N4 部分实现，N5 正式发布
迁移未完成。构建命令见 [native/README.md](native/README.md)，完整边界与验收见
...
粘贴操作由 Swift/AppKit 负责；当前 Rust 桌面层的这些能力需要迁移，不能随 Tauri
一起删除。所有业务判断、模型适配、动作格式与渲染编排留在 Core，CLI 继续保留。
...
### N5 原生交付与移除旧桌面层（本地打包已实现，正式发布未完成）

切换 CI、打包脚本、原生更新机制、签名公证、发布和安装文档；明确旧 Tauri 版本如何
升级至原生版本。实际打开产物并完成端到端验收后，移除 Tauri/WebView 前端、Rust
桌面层及其专用依赖、配置和流水线。引擎依赖的 Rust/WASM 不在删除范围。
交付准确的 `.app` 路径、版本和验证记录，不能以网页预览或 `vite build` 代替验收。
...
- 挑选第一版是预选加确认，不自动粘贴。
- SwiftUI + AppKit 负责原生桌面层，最终移除 Tauri 与界面 WebView，不引入 Electron。
