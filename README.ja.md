...
- **プレーンテキストからカードに。** 19 のテンプレートと 39 のスタイル。言葉、名前、数字、順番はすべて元のテキストのまま。書き換えも付け足しもしません。
...
<img src="docs/images/chooser-ja.png" width="360" align="right" alt="「形式を選んでペースト」パネル：カードのプレビューの下に、画像（Return）、GIF（G）、ビデオ（M）、QR コード（Q）、画面にピン留め（P）、クリップボード履歴（H）。">

テキストをコピーして、入力中の場所で **⌥V** を押します。カーソルの位置に小さなパネルが開き、カードはもうできています。あとはキーをひとつ：

| キー | 出力 |
|---|---|
| **Return** | 画像：PNG を、いま入力中のアプリにそのままペースト |
| **G** | GIF：同じカードを、一行ずつ表示 |
| **M** | 動画：MP4。Mac に入っている ffmpeg で作ります |
| **Q** | QR コード：コピーしたテキストをそのまま。モデルには送りません |
| **P** | 画面にピン留め：すべてのウィンドウの上に浮かびます。ドラッグで移動、ピンチで拡大、ダブルクリックで閉じます |
| **H** | クリップボード履歴（**⇧⌥V** でも開けます） |

画像、GIF、動画、QR コード、ピン留めには「設定 › ショートカット」で個別のキーも割り当てられます。初期状態では未設定です。
...
<p align="center">
  <img src="docs/images/templates.png" width="100%" alt="39 のカードスタイル：テキスト、ドキュメント、引用、コード、数字、リスト、会話、表、比較、図、情報カード、リリースノート、ターミナルセッション、差分、エラー、スケジュール、指標、歌詞、QR コード。">
</p>

テキスト · ドキュメント · 引用 · コード · 数字 · リスト · 会話 · 表 · 比較 · 図 · 情報カード · リリースノート · QR コード
...
**動作環境：** Apple シリコン搭載で、macOS 13 Ventura 以降の Mac。[ffmpeg](https://ffmpeg.org)（`brew install ffmpeg`）は MP4 動画を作るときだけ必要で、Peesuto は同梱も自動インストールもしません。
