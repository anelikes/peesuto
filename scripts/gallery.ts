...
import { ALL_FORMATS, errorOf, planText, renderJob, type JobGroup } from "./studio/pipeline.ts";
...
const GROUP_PREFIX: Record<JobGroup, string> = { chosen: "", other: "可选：", motion: "", video: "" };
...
for (const scenario of SCENARIOS.filter((s) => !only || only.includes(s.id))) {
  ...
  try {
    const { decision, jobs } = await planText(scenario.text, { aspect: "chat", decider: "rules", answersDir: join(out, ".answers"), formats: ALL_FORMATS, video: video && Boolean(ffmpeg) });
    ...
    for (const job of jobs) {
      const file = `${scenario.id}-${job.template}-${job.variant}-${job.motion}.${job.format}`;
      try {
        const r = await renderJob(job.plan, job.format, join(out, file), engineOptions);
        const note = [`${r.width}×${r.height}`, r.frames > 1 ? `${r.frames} 帧` : "", r.scroll ? "滚动" : "", `${r.ms} ms`].filter(Boolean).join(" · ");
        section.shots.push({ label: GROUP_PREFIX[job.group] + job.label, file, kind: job.format, group: job.group, note });
...
<header><h1>Peesuto 模板预览</h1><p>每个场景按应用的本地规则自动选模板（第一组），其后是同一模板的其他风格、其他可选模板的各风格、每种动效的 GIF 和一段视频。生成于 ${new Date().toLocaleString("zh-CN")}。</p></header>
