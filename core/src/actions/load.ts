...
export function parseActionSpec(raw: unknown, origin = "action"): ActionSpec {
  ...
  if (r.needs === "render") {
    ...
    if (ren.outputs !== undefined && (!Array.isArray(ren.outputs) || ren.outputs.length === 0 || !ren.outputs.every((o) => ["image", "gif", "video"].includes(o as string)) || !ren.outputs.includes(r.output))) bad("render.outputs must list image, gif or video, including output");
    const canVideo = r.output === "video" || (Array.isArray(ren.outputs) && ren.outputs.includes("video"));
    if (ren.fallback !== undefined && (ren.fallback !== "gif" || !canVideo)) bad("render.fallback must be gif, on an action that can make video");
  }
