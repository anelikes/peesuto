...
### New
- **Lyric motion (文字 PV)** — the lyrics template becomes Lyric motion and works on any text, not only lyrics: press **L** in Paste as… (⌥V) for kinetic type of whatever you copied. Prose is cut into screens at its sentences and clauses (never mid-word), held long enough to read; lyrics, poems and LRC keep their own lines. It makes a GIF by default, or a video or a poster (see the next item). It is never chosen automatically any more, so copied lyrics and poems now come out as text or document cards unless you ask; switch any result to Lyric motion from its template menu, or give it its own shortcut in Settings › Shortcuts. Code, tables and very long text get a clear message instead of a video.
- **Lyric motion's output** — Settings › Templates › Lyric motion › Default output: **GIF** (the default), **Video** or **Poster (PNG)**. The chooser row says which ("Lyric motion · GIF"), and a result's format menu still switches one result. Text too long for a GIF (about 14 seconds) says so and suggests Video instead of switching silently.
- **Video without ffmpeg** — MP4 is now encoded by Peesuto itself with the Mac's hardware H.264 encoder (AVFoundation and VideoToolbox), so Video (M) and Lyric motion videos work out of the box, with nothing to install. The encoder is more than twice as fast as the ffmpeg pass it replaces, the file opens progressively (moov first), and colours now match the image cards in QuickTime, Safari and Messages (BT.709, tagged sRGB; ffmpeg's untagged output shifted saturated reds and lifted greys). ffmpeg is no longer used by the app; `bun run paste` outside the app still falls back to it.

### Improved
- **Lyric motion, redrawn** — four styles instead of two: **Stage** (heavy 得意黑 / Dela Gothic / Anton type, colour cuts on black, red, cobalt and yellow), **Paper** (霞鹜文楷 / Kaisei Tokumin / Instrument Serif, vertical CJK, hairlines, a vermilion seal), **Pop** (站酷快乐体 / Zen Maru Gothic, candy colours, stickers, bouncy motion) and **Night** (DotGothic16 / 得意黑, colour ghosts, glitch cuts, scanlines, a camera HUD). Each cut now picks one of 22 compositions (a giant word filling the frame, words stacked at jumping sizes, a tilted band, split colour fields, hollow ghost repeats, word plates, tickers, a big number, an editorial corner caption and more), lines land in two or three hits, Pop and Night move on twos like anime (12 drawings a second), cut lengths follow a beat, and fine film grain lies over the picture. Words larger than before are drawn by Peesuto itself, so a short line can fill the screen. The text is still drawn exactly once, readable and in high contrast; the display fonts (OFL, about 13 MB) ship with the app.
...
### Improved
- **A smaller download** — the app is about 45 MB smaller (the DMG about 11 MB), with every template still rendered offline.
- **Install with Homebrew** — `brew install --cask anelikes/tap/peesuto`.
