...
export class Daemon {
  ...
  private keyOf(spec: ActionSpec, input: { text: string; aspect?: string; templatePreferences?: Readonly<Record<string, string>>; disabledTemplates?: readonly string[]; templateFont?: string }): string | null {
      ...
      switch (req.cmd) {
        ...
        case "precompose": {
          ...
          for (const output of this.precompose.outputs) {
            ...
            const input = { text: req.text, ...(frames[output] ? { aspect: frames[output] } : {}), ...(req.templatePreferences ? { templatePreferences: req.templatePreferences } : {}), ...(req.disabledTemplates ? { disabledTemplates: req.disabledTemplates } : {}), ...(typeof req.templateFont === "string" ? { templateFont: req.templateFont } : {}) };
