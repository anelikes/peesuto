...
export async function decideCard(text: string, o: { aspect: Aspect; decider: CardDecider | null; force?: { animate?: boolean; minTone?: Level } }): Promise<CardDecision> {
  const t0 = performance.now();
  let dsl: Dsl, decided: CardDecision["decided"];
  const answers = o.decider ? await o.decider.ask(buildRequest(text).body) : null;
  if (isCardAnswers(answers)) {
    const r = answersToDsl(text, answers, o.aspect);
    dsl = r.dsl;
