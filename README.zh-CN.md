...
<img src="docs/images/chooser-zh.png" width="360" align="right" alt="“粘贴为…”面板：上方是卡片预览，下面是图片（I，已高亮，回车即选）、GIF（G）、视频（M）、文字 PV（L）、二维码（Q）、贴到屏幕（P）和剪贴板历史（H）。">

复制一段文字，在任何输入的地方按 **⌥V**。光标处会打开一个“**粘贴为…**”的小面板，卡片已经画好。再按一个键；或按**回车**选高亮的那一行（默认是图片，可用方向键移动）：
...
图片、GIF、视频、文字 PV、二维码和贴到屏幕也可以在“设置 › 快捷键”里各自绑定快捷键，默认不绑定。
...
**系统要求：** 搭载 Apple 芯片、运行 macOS 13 Ventura 或更高版本的 Mac。不需要安装其他组件：视频由 Mac 内置的 H.264 编码器生成。
