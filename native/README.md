...
预览模式的菜单栏直接动作禁用，「粘贴为…」只打开示例文字的选择器（不读剪贴板）；快捷键设置可录入和校验，但不注册系统快捷键。
...
默认绑定：`⌥V` 打开「粘贴为…」选择器，`⇧⌥V` 打开剪贴板历史。图片、GIF、视频、二维码、
贴到屏幕五个直接快捷键默认不绑定，需要时在「设置 → 快捷键」单独设置（跳过选择器）。
默认值集中在 `PeesutoKit/Settings.swift` 的 `DefaultShortcuts`，改一行即可。
`settings.json` 的 `shortcuts_version` 低于当前版本（或缺失）时，启动会把全部绑定重置为默认值一次
（不保留旧的自定义绑定），并在面板提示新按键。
在「设置 → 快捷键」点击对应按钮后按键录入，保存生效；清除即可停用。
如果系统注册冲突，设置保留旧绑定并报错；启动时冲突会提示，并尽量保留能注册的绑定（选择器与历史优先）。

### 「粘贴为…」选择器（⌥V）

在别的应用里输入时按 ⌥V，光标旁出现一个小浮窗（不激活 Peesuto，目标应用保持在前台，
最后的 ⌘V 落回原处）。位置取自辅助功能：焦点元素的选区范围 → 该范围的屏幕边框；
取不到时放在鼠标附近，再不行放在屏幕上三分之一处。浮窗上方是即将粘贴的卡片缩略图：
按「粘贴为图片」完全相同的请求向 Core 要一次（同一动作、画幅、模板偏好、停用模板、字体、签名），
复制后预先生成过就立即命中缓存，否则显示进度圈边渲染边可按键；选图片或贴到屏幕时直接复用这次渲染。
下方是单键选项：回车 图片、G GIF、M 视频、Q 二维码、P 贴到屏幕、H 打开完整历史、Esc 取消；
也可点击。选择后浮窗关闭，执行与对应直接快捷键完全相同的流程（贴到屏幕走贴图投递）。
点击别处、失去焦点或 Esc 关闭且不做任何事。剪贴板没有文字（例如是图片）时生成类选项置灰并说明，
P 仍可贴出图片，H 仍可用。未授予辅助功能或处于密码输入时只复制并说明原因，与直接快捷键一致。
按键映射与可用性规则在 `PeesutoKit/PasteChooser.swift`，有单元测试。
预览版截图参数：`--chooser-sample`（示例文字）、`--chooser-image`（剪贴板为图片）。

先复制文字，再在目标输入框按快捷键。应用读取当前剪贴板快照，显示非抢焦点的状态浮窗，
生成后把结果放上剪贴板并直接对当前最前面的应用按 ⌘V，这是唯一行为，没有开关。
只在三种情况下仅复制并说明原因：未授予辅助功能（浮窗提供「授予权限…」）、处于密码框等
安全输入状态、最前面的是 Peesuto 自己。结果留在剪贴板上，不恢复原剪贴板。

「复制后预先生成」（设置 → 快捷键，与「隐私与预合成」同一份设置）：复制文字后在后台用本地
规则提前生成，快捷键命中即刻粘贴。新安装默认只预生成图片；低电量模式下暂停。

### 贴到屏幕（选择器里的 P）

类似 Snipaste 的贴图。剪贴板里是图片（PNG/TIFF 数据、GIF，或指向图片文件的文件 URL；
Peesuto 自己写入的也算）时直接贴出，不渲染；否则如果是文字，就按「粘贴为图片」完全相同的路径
生成图片卡片（同一图片画幅与模板偏好，所以复制后预先生成的结果会立即命中），渲染期间显示
同样的状态浮窗，然后把结果贴出而不是粘贴。这个动作从不按 ⌘V，也从不改动剪贴板。
菜单栏「直接粘贴为」子菜单和结果视图（图片与 GIF，视频不支持）也有「贴到屏幕」。

贴图是无边框、不抢焦点的浮动窗口（所有桌面空间与全屏应用上可见），初始为图片的自然点尺寸
（像素 ÷ 屏幕缩放），缩到可见屏幕宽高的 45% 以内，以鼠标为中心并限制在屏幕内。拖动任意处
移动；滚轮或双指滚动以光标为中心平滑缩放（短边不小于 64 pt，不超过自然尺寸的 400% 或屏幕
大小），触控板捏合同样缩放；双击淡出关闭；右键菜单：复制图片、另存为…（按来源存 PNG 或
GIF）、关闭、关闭全部贴图。GIF 会播放。可同时存在多个贴图，重启后不恢复。几何计算在
`PeesutoKit/Pin.swift`（`PinGeometry`）并有单元测试。预览版可用 `--pin-sample` 启动参数
贴出两张示例图用于截图。

首次启动会显示欢迎引导（五步：欢迎、快捷键、权限与隐私、偏好、试一试），之后可在
「设置 → 通用 → 关于」重新查看。
