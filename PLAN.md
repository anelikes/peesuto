...
已打通加密历史、系统集成、Bun 通信、图片/GIF/视频动作与中英文；视频由随包 PeesutoEncoder（AVFoundation）编码，不需要 ffmpeg。
...
- **图片、GIF、视频生成是核心能力。** 保留 Pocket Motion 的画面描述、动画与确定性
  帧渲染能力；当前已接入 PNG/GIF/MP4。MP4 由应用内的 PeesutoEncoder（AVFoundation +
  VideoToolbox 硬件编码）生成，随应用签名分发；ffmpeg 只是应用外 CLI 的后备。
...
- **模型双轨、三种接入。** 决策类（Jev 或同类小模型）和生成类（LLM）各自独立
  ...
  base URL 加可选凭证。
- **全部开源，先做纯 BYOK。** 应用、Core、proxy、模板与包格式全部 MIT，
  不做阉割版。现阶段只提供本地与自带 key；托管服务（免配置的模型调用、同步、
  分享链接等）代码保留、暂不上线，等社区有需求再开。商标 Peesuto 不随许可证授予。
...
7. **核心输出完整。** PNG/GIF 保持既有渲染结果；视频动作已接入 MP4，由随包
   PeesutoEncoder 编码（0.2.2 起不再需要 ffmpeg）。目标应用兼容验收仍是独立交付项。
...
渲染链；`paste-video` 和 `run-action` 已支持视频：Core 在进程内渲染帧，经 stdin
把 RGBA 交给随包的 PeesutoEncoder 输出 MP4；没有它时（应用外 CLI）退回本机 ffmpeg，
两者都没有时明确报错，不影响 PNG/GIF。
...
快捷键设置。默认 ⌥V 打开「粘贴为…」选择器（图片/GIF/视频/二维码/贴到屏幕各一个键），
⇧⌥V 打开历史；各输出也可单独绑定。直接使用当前剪贴板文字，无需
...
误退出，任务完成后才重新计时。视频内置动作已接入（随包编码器）；协议仍串行，
...
### M8 订阅（代码侧已完成 2026-09-22；2026-09-24 起暂缓上线，见 §5）
...
- 许可证由 MIT 改为 Apache 2.0（2026-09-24 用户，同日已被下一条取代）：看重专利授权、贡献默认同许可（§5 条款，
  配 DCO 签名，不设 CLA）和商标不授予（§6 条款）。护城河放在品牌、社区与模板生态、迭代速度，
  而不是代码；AGPL 被否，因为它劝退贡献者与公司，而挡不住重写。
- 许可证改回 MIT（2026-09-24 用户）：更简单，生态里更常见（如 ljg-skills、tw93/kami 均为 MIT）；
  保留商标政策与 DCO 签名（贡献按 MIT 进出同许可）；Apache 的明示专利授权判断为不必要。
- BYOK 决策服务（2026-09-24 用户）：Jev 可走 Cloudflare、TypeSafe 官方、Vercel AI Gateway、OpenRouter，
  都用用户自己的密钥；OpenRouter 与 Vercel 的同一密钥也用于文本生成。引导与设置都可配置。
- 商业化方向（2026-09-24 用户）：面向海外；先完全开源、纯本地与 BYOK、打赏赞助，
  账号系统与托管订阅暂缓，proxy 的订阅代码保留。以后按需求再开托管（open-design 式：
  自带 key 或买官方额度，加同步、分享链接、团队品牌套件等服务端增值）。
  长期目标是社区模板创意工坊，前提是声明式或沙箱化的模板格式。
- PNG/GIF/MP4 都不依赖 ffmpeg：MP4 由随包 PeesutoEncoder（AVFoundation）编码，
  ffmpeg 只在应用外的 CLI 中作为后备（或 PEESUTO_VIDEO_ENCODER=ffmpeg 调试）。
...
- 2026-09-23 模板视觉重做（Claude Design 方案）：中性纸墨加每种模板一种签名色，全部模板参数表化，内容少时放大一档再居中。
  设计方提出、需要引擎或字体支持的项目记在这里，未开始：等宽字体（代码）、思源宋体（书摘/专栏/引言）、Medium 字重、
  等宽数字 tnum、大字字距收紧、每角独立圆角或裁切、原生描边、SVG 非等比缩放或多边形图元（菱形边框粗细）。
