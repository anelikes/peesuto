...
describe("code font", () => {
  test("Maple Mono by default for every template, Noto Sans SC when chosen (never for the monospace templates) or a glyph is missing", () => {
    expect(MONO_TEMPLATES).toEqual(["code", "terminal", "diff", "error"]);
    for (const id of TEMPLATE_IDS) {
      // Code, shell sessions, diffs and errors keep Chinese at two columns; every other card uses Peesuto Text (Chinese at 1em).
